...
\begin{proc}{histograma}{\In xs: \TLista{Viaje},\In bins: \ent,out cuentas: \TLista{\ent},out limites: \TLista{\float}}{}
	...
	\pred{esVelocidadMaxima}{\In i: \ent,\In v: Viaje}{
		...
		\pred{limitesValidos}{\In limites: \TLista(\float), \In bins: \ent, \In xs: \TLista{Viaje}}{
			\longitud{limites} = \longitud{bins} + 1 \wedge
			(\forall i: \ent)(0 \leq i < \longitud{limites} -1 \wedge limites[i] > limites[i+1]) \wedge 
			(\exists velMayorMaxima,velMayorMinima: \ent)(\forall j:\ent)(0 \leq j < \longitud{xs}  \yLuego %Chequear que sean velMaxima )
			}
