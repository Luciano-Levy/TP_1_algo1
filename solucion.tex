...
\begin{proc}{flota}{\In v: \TLista{Viaje},\In t_{0}: Tiempo, \In t_{f}: Tiempo,  \Out res: \ent}{}
	...
	\post{{\sum_{i=0}^{|v|-1}}  \IfThenElse {(viajeEnElTiempoPedido (v[i], t_{0}, t_{f}))}{1}{0}}
...
\begin{proc}{cantidadDeSaltos}{\In g: Grilla,\In v: Viaje \Out res: \ent}{}
	...
	\post{res = sumSaltos(v,g) div 2}
	 
		
	\pred {esUnSalto} {in g: Grilla, in v: Viaje}{ 
		(\exists i,j : \ent)(0 \leq i, j < |v|)  v[j]_{0} -  v[i]_{0} = 20 \yLuego ((numeroFila(v[j]_{1}) - numeroFila(v[i]_{1}) \geq 3) \vee (numeroCol(v[j]_{1}) - numeroCol(v[i]_{1}) \geq 3)) }
		
	aux sumSaltos (in g: Grilla, in v: Viaje) {
	  \sum_{i=0}^{\longitud v - 1} \IfThenElse {esUnSalto(g,v)}{1}{0}
	}	
\end{proc}	
